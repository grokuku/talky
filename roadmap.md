...
- **Zoom global automatique (fit-vp)** ✅ : la page s'adapte à la hauteur de la fenêtre **sans scroll** — zoom CSS auto borné [0.85, 1.6] sur le conteneur `.layout` (repli transform si `zoom` non supporté), re-calcul debouncé (150 ms) uniquement sur first-run / resize / fonts.ready + invalidation one-shot au premier renderConfig ; ré-engagement via FIT_VP_MQ ; colonne config scrolle en interne, colonne gauche compacte. Sans JS → scroll normal conservé.
